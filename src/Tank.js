...
Tank.prototype.getPlayerNumber = function () {
  if (this._type == Tank.Type.PLAYER_2) {
    return 2;
  }
  return 1;
};

Tank.prototype.setKiller = function (killer) {
  this._killer = killer;
};

Tank.prototype.getKiller = function () {
  return this._killer;
};
...
Tank.prototype.setTeammate = function (teammate) {
  this._teammate = teammate;
};

Tank.prototype.isTeammate = function () {
...
Tank.prototype.shoot = function () {
  ...
  if (!(this._player && Cheat.infiniteBullets) && this._bullets >= this._bulletsLimit) {
    return;
  }
  this._bullets++;
...
Tank.prototype.notify = function (event) {
  ...
  else if (this._bulletCollision(event) && this.canBeDestroyed() && !this._cheatInvincible()) {
    this._killer = event.initiator.getTank();
    this.hit();
  }
...
Tank.prototype.hit = function () {
  this._hit++;
  this._color.hit();
  if (this._hit == this._hitLimit || (this.isEnemy() && Cheat.oneHitKill)) {
    this.destroy();
  }
};
...
Tank.prototype._cheatInvincible = function () {
  return this._player && Cheat.invincible;
};
