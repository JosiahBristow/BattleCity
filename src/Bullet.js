...
Bullet.prototype.notify = function (event) {
  ...
  else if (this._baseCollision(event) && !this._tank.isTeammate()) {
    this.destroy();
  }
...
Bullet.prototype._tankCollision = function (event) {
  ...
  // AI teammate does not hurt player 1 (friendly fire off for teammate)
  if (this._tank.isTeammate() && otherTank.isPlayer()) {
    return false;
  }
  return true;
};
