...
Pause.Event.QUIT = 'Pause.Event.QUIT';
...
Pause.prototype.keyPressed = function (key) {
  ...
  if (key == Keyboard.Key.ESC) {
    this._eventManager.fireEvent({'name': Pause.Event.QUIT});
    return;
  }
  if (key == Keyboard.Key.START || key == Keyboard.Key.P) {
...
Pause.prototype.draw = function (ctx) {
  ...
  ctx.fillText(Language.translate('PAUSE'), 202, 240);
};
