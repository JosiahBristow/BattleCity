describe("PointsFactory", function () {
  ...
  describe("#notify", function () {
    describe("TankExplosion.Event.DESTROYED", function () {
      it("enemy tank - 100 points", function () {
        ...
        expect(factory.create).toHaveBeenCalledWith(explosion.getCenter(), tank.getValue(), Points.Type.TANK, undefined);
      });
      ...
      it("power-up", function () {
        ...
        expect(factory.create).toHaveBeenCalledWith(powerUp.getCenter(), powerUp.getValue(), Points.Type.POWERUP, undefined);
      });
    });
  });
  ...
  it("#create", function () {
    ...
    expect(eventManager.fireEvent).toHaveBeenCalledWith({'name': PointsFactory.Event.POINTS_CREATED, 'points': points, 'playerNumber': undefined});
  });
});
